...
let stateVar = {};
const dialogStyles = { main: { minWidth: 1000, border:'5px solid #000' } };
/* const config = {
  server: 'localhost',
  authentication: {
    type: 'default',
...
const filter = (text: string): any[] => {
  return ITEMS.filter(i => i.Status.toLowerCase().indexOf(text.toLowerCase()) > -1) || ITEMS;
}
    ...
    constructor(props){  
     ...
     /*  itemsData:tableItemsData, */
      ...
      currentStep:'Line Items',
	    MonthlyReportMonthStart:"",
      MonthlyReportMonthEnd:"",
      MonthlyReportData:[],
      firstDayOfWeek:DayOfWeek.Sunday,
      showLoadTemplateDialog:false,
      showAddTemplateDialog:false,
      displayReportviewModal:false,
      ...
      tax:'1000',
      total:'',
	    CustomerReportCutomer:'',
	    ProductReportProduct:'',
	    MonthlyReportMonth:'',
      Qaddress1:'',
      customerList:[],
      addEsign:true,
	  ...
	  this.sendQuotationEmail = this.sendQuotationEmail.bind(this);
	  ...
	  this.handleProductReportSection = this.handleProductReportSection.bind(this);
	  this.handleMonthlyReportSection = this.handleMonthlyReportSection.bind(this);
	  this.handleMonthlyReportChange = this.handleMonthlyReportChange.bind(this);
    this.handleProductReportChange = this.handleProductReportChange.bind(this);
    this.handleMonthlyReportView = this.handleMonthlyReportView.bind(this);
      ...
      axios.get('http://localhost:9000/testAPI/customers').then((res) => {
            ...
            res.data.forEach(function(bar, index) {
              ...
              var custItem = {key:bar.customer,text:bar.customer};
      ...
      axios.get('http://localhost:9000/testAPI/items').then((res) => {
            ...
            res.data.forEach(function(bar, index) {
              ...
              var Item = {key:bar.Description,text:bar.Description};
              itemList.push(Item);
              tmp.Spec = bar.Spec;
      ...
      popover = (
        <Popover id="popover-basic">
          <Popover.Title as="h3"  style={{backgroundColor:'grey'}}>Options</Popover.Title>
          <Popover.Content>
 ...
 */
  ...
  handleMonthlyReportView = (index,items) => {
    /* if(this.state.displayReportviewModal == true){
      return false;
    } */
    //alert('handleMonthlyReportView '+index+'  '+this.state.displayReportviewModal);
    ////this.setState({displayReportviewModal:!this.state.displayReportviewModal})
  }

  handleCustomerReportSection = () => {
    axios.post('http://localhost:9000/testAPI/customerReport',{customer: this.state.CustomerReportCutomer}).then((response) => {this.setState({MonthlyCustomerData:response.data})});
    this.setState({
  ...
  handleProductReportSection = () => {
    axios.post('http://localhost:9000/testAPI/productReport',{product:this.state.ProductReportProduct }).then((response) => {this.setState({MonthlyProductData:response.data})});
    this.setState({
  ...
  handleMonthlyReportSection = () => {
    axios.post('http://localhost:9000/testAPI/monthlyReport',{MonthlyReportMonthStart:this.state.MonthlyReportMonthStart,MonthlyReportMonthEnd:this.state.MonthlyReportMonthEnd}).then((response) => {/* alert(JSON.stringify(response.data)); */this.setState({
      MonthlyReportData:response.data,
      handleMonthlyReportSection:!this.state.handleMonthlyReportSection
    })});
    ...
    handleCutomerReportChange = (e) => {
      ...
      this.setState({
        /* Currency: e, */
		      CustomerReportCutomer:e
      })
    }
	
	handleProductReportChange = (e) => {
      ///console.log(util.inspect(e))
      ///var inst = e._targetInst._debugOwner.key;
      alert(e);
      this.setState({
		      ProductReportProduct:e
      })
    }
	
	handleMonthlyReportChange = (e) => {
		var rMonth = document.getElementById('monthreport').value;
      ///console.log(util.inspect(e))
      ///var inst = e._targetInst._debugOwner.key;
      this.setState({
		MonthlyReportMonthStart:new Date(rMonth+"-01").getTime(),
		MonthlyReportMonthEnd:new Date(rMonth+"-31").getTime()
      })
    }
...
<table id='myTable-Header'>
   <tbody>
      ...
      <td>{this.state.payTerm}</td>
          ...
          <Dialog hidden={!this.state.displayReportviewModal} onDismiss={!this.state.displayReportviewModal} modalProps={modelProps} style={dialogStyles}>
            <Stack>
                  <DetailsList
                  ...
                  />
                  <PrimaryButton text="CANCEL" onClick={() => {this.setState({displayReportviewModal:!this.state.displayReportviewModal})}}></PrimaryButton>
            </Stack>
              ...
              <Stack horizontal tokens={stackTokens} styles={stackStyles}>
                <Stack {...columnProps}>
                  ...
                  <DatePicker
                    ...
                    onChanged={this.handleQouteDateChange}
                  />
                  {/* <FDropdown
                    placeholder="Select an option"
                    label="Sales Rep."
                  ...
                  <Stack horizontal tokens={stackTokens}>
					...
					<PrimaryButton text="Email" onClick={this.sendQuotationEmail()} allowDisabledFocus/>
                ...
                <div class="reports-submenu-wrapper" onClick={ (e) => this.handleReportsSection}>
                  <SubMenu title="Reports" icon={<FaVoteYea/>}>
                    ...
                    <SubMenu title="Customer Wise Report" >
                      <div onClick={this.handleCustomerReport}>
                        <FDropdown
                    ...
                    <div  onClick={this.handleProductReport}>
                      <FDropdown
                      ...
                      required
                      /* styles={dropdownStyles} */
                      onChanged={this.handleProductReportChange}
                      />
                      <DefaultButton text="Generate"  onClick={this.handleProductReportSection}/>
                    </div>
                    </SubMenu>
                    <SubMenu title="Monthly Report">
                    <div  onClick={this.handleMonthlyReport}>
					            <input type="month" id="monthreport" name="bdaymonth" onChange={this.handleMonthlyReportChange} ></input>
                      <DefaultButton text="Generate" onClick={this.handleMonthlyReportSection}/>
                    </div>
                    </SubMenu>
            ...
            {/* <Table celled selectable sortable green class="ui green table" color="green">
              ...
              {this.state.handleQuotationSection ? <Table  responsive celled selectable sortable /* className="quoteTable mainTable" */>
                ...
                <Table.Row>
                  ...
                  {this.state.handleMonthlyReportSection ? <Table  responsive celled selectable sortable /* className="quoteTable mainTable" */>
                
                <Table.Row>
                    <Table.HeaderCell>Quotation No.</Table.HeaderCell>
                    <Table.HeaderCell>Date</Table.HeaderCell>
                    <Table.HeaderCell sorted={column === 'unit' ? direction : null}
                  onClick={this.handleSort('unit')}>Customer</Table.HeaderCell>
                  <Table.HeaderCell sorted={column === 'unit' ? direction : null}
                  onClick={this.handleSort('unit')}>payTerm</Table.HeaderCell>
                  <Table.HeaderCell></Table.HeaderCell>
                  </Table.Row>
                    <Table.Body>
                      {_.map(this.state.MonthlyReportData, ({Number, date, quotTo, payTerm, items },index) => (
                            <Table.Row key={Number} onClick={this.handleQoutationRowClick(index,quotationData[index])}>
                              <Table.Cell>{Number}</Table.Cell>
                              <Table.Cell>{date}</Table.Cell>
                              <Table.Cell>{quotTo}</Table.Cell>
                              <Table.Cell>{payTerm}</Table.Cell>
                              <Table.Cell><PrimaryButton text="VIEW" name={items} id={"reportView"+index} onClick={() => {/* alert(JSON.stringify(items)); */this.setState({displayReportviewModal:!this.state.displayReportviewModal,ITEMS:items})}/* this.handleMonthlyReportView(index,items) */} allowDisabledFocus/></Table.Cell>
                            </Table.Row>
                      ))}
                    </Table.Body>
                  ...
                  {this.state.handleCustomerReportSection ? <Table  responsive celled selectable sortable /* className="quoteTable mainTable" */>
                ...
                <Table.Row>
                  ...
                  <Table.HeaderCell></Table.HeaderCell>
                    ...
                    <Table.Body>
                      {_.map(this.state.MonthlyCustomerData, ({Number, date, quotTo, payTerm, items },index) => (
                            <Table.Row key={Number} onClick={this.handleQoutationRowClick(index,quotationData[index])}>
                              <Table.Cell>{Number}</Table.Cell>
                              <Table.Cell>{date}</Table.Cell>
                              <Table.Cell>{quotTo}</Table.Cell>
