...
class App extends Component {
    ...
    onFileUpload = () => { 
      ...
      var fileObj = this.state.selectedFile;
 ...
 */
    ...
    onLineItemChange = (index,value,el) => {
      ...
      if(el == 'itemID'){
        rows[index] = {
          id:value,
          qty:$('#itemQty'+(index+1)).val(),
          unit:$('#itemUnit'+(index+1)).val(),
          unitPrice:$('#itemUnitPrice'+(index+1)).val(),
          total:$('#itemTotal'+(index+1)).val(),
          desc:$('#itemDesc'+(index+1)).val()
        }
      }else if(el == 'itemDesc'){
        rows[index] = {
          id:$('#itemID'+(index+1)).val(),
          desc:value,
          unit:$('#itemUnit'+(index+1)).val(),
          unitPrice:$('#itemUnitPrice'+(index+1)).val(),
          qty:$('#itemQty'+(index+1)).val(),
        ...
        rows[index] = {
          ...
          unitPrice:$('#itemUnitPrice'+(index+1)).val(),
          qty:value,
          total:parseFloat(rows[index].unitPrice)*parseFloat(value)
        }
       // alert(parseFloat(rows[index].unitPrice)*parseFloat(rows[index].qty)+'  '+rows[index].unitPrice+' '+rows[index].qty)
        ...
        rows[index] = {
          ...
          unitPrice:value,
          qty:$('#itemQty'+(index+1)).val(),
          total:$('#itemTotal'+(index+1)).val()//parseFloat(rows[index].unitPrice)*parseFloat(rows[index].qty)///$('#itemTotal'+(index+1)).val()
        }
    ...
    handlePDFPreview = () => () => {
        ...
        console.log('my-table '+util.inspect(table));
        console.log('clone my-table '+util.inspect(cloneTable));
        ...
        autoTable(doc, { html: table, theme:'grid',startY:105,alternateRowStyles:true,headStyles:{lineWidth:0.3,lineColor:[0,0,0]},bodyStyles:{lineWidth:0.3,lineColor:[0,0,0]},footStyles: {cellWidth: 'wrap',
        /* fontSize: 18, */
        lineWidth: 0.3,
        lineColor: [0, 0, 0],
        textColor: [0, 0, 0],
        halign:'right',
        fillColor: [255,255,255]},alternateRowStyles:true,styles: {overflow: 'linebreak', columnWidth: '100', font: 'Calibri', fontSize: 10, cellPadding: 1, overflowColumns: 'linebreak'}})
        console.log(_QuotationNotesObj);
        autoTable(doc, { html: "#myTable-Notes", theme:'grid',startY:175,/* rowStyles:{1:{backgroundColor:'red'}} *//* bodyStyles: {halign: 'center', fillColor: [151, 154,  170]}, */ columnStyles: {1:{cellWidth: 'wrap',
                  fontSize: 18,
                  lineWidth: 2,
    ...
    handletotalDiscountChange = () => {
      ...
      alert('handletotalDiscountChange');
    ...
    itemTotalUpdate = (qty) =>  {
      ...
      for (index = 0; index < itemQty.length; index++) {
        ...
        if(qty == itemQty[index].value){
          document.getElementsByClassName('itemTotal')[index-1].value = lineItemTotal;
        }
      }
...
<> */        <div>
  <table id="my-table">
    <thead style={{borderStyle:'none'}}>
      <tr style={{fillColor:[211,211,211]}}>
        <th>ID</th>
        <th>Description</th>
        <th>Unit</th>
        <th>UnitPrice</th>
  ...
  <tfoot>
  <tr>
        <td colSpan='5' style={{textAlign:'right',border:'none',border:'0px solid #fff',color:'red',fontWeight:'bolder'}}>Sub-Total ({this.state.Currency}) {/* <br></br>156.00 */}</td>
